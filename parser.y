...
%token IF ELSE DO WHILE RETURN VOID INT FLOAT CHAR
...
%expect 2 
%type<str> atree program external_declaration var_declaration init_declarator_list fun_declaration params_list compound_stmt declarator params block_item_list block_item call factor term additive_expression simple_expression unary_expression postfix_expression assignment_expression return_stmt do_while_stmt if_stmt else_if expression statement args expression_stmt 
%type<str> relop declaration_specifiers input_output op array_init arrayindex 
...
printf
	: PRINTF LPAREN print_out RPAREN SEMI
print_out
	: op {if(errors==0) printf("%s", $1);}
	| op print_out {if(errors==0) printf("%s", $1);}
	| op ENDL {if(errors==0) printf("%s\n", $1);}
...
do_while_stmt
    : DO {
        ...
        strcpy(arg1,$7);
        strcpy(false_label,create_label());
        new_record = create_quadruple(statement_type,arg1,"","",$<str>2, yylineno);
        insert_quadruple(q_list1,new_record);
